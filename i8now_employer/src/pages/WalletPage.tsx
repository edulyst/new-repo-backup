...
export function WalletPage() {
  ...
  async function load() {
    ...
    try {
      const [wRes, tRes] = await Promise.all([
        api.get<{ data: { wallet: WalletData } }>('/employer/wallet'),
        api.get<{ data: Transaction[] }>('/employer/wallet/transactions?limit=30'),
      ])
  ...
  async function handlePay() {
    ...
    setPaying(true)
    try {
      await api.post('/employer/wallet/pay', {
        ...
        {loading ? (
          ...
          <>
            <div className="rounded-xl border border-zinc-200 bg-zinc-900 p-5 text-white">
              ...
              <p className="mt-0.5 text-xs text-zinc-500">{wallet?.currency ?? 'INR'}</p>
            </div>
                ...
                <tr key={t._id} className="hover:bg-zinc-50/60 transition-colors">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn('text-xs font-medium capitalize', t.type === 'credit' ? 'text-emerald-700' : 'text-red-600')}>
                        {t.type}
                      </span>
                    </div>
                  </td>
          ...
          <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input value={payForm.worker_id} onChange={(e) => setPayForm({ ...payForm, worker_id: e.target.value })}
                  placeholder="Paste worker's user ID" className={inputCls} />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <input value={payForm.description} onChange={(e) => setPayForm({ ...payForm, description: e.target.value })}
                  placeholder="e.g. Weekly shift payment" className={inputCls} />
              </div>
              <div className="flex gap-2 pt-2">
                <button onClick={() => setShowPay(false)} className={cancelBtn}>Cancel</button>
                <button onClick={handlePay} disabled={paying} className={primaryBtn}>
