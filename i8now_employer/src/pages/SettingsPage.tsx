...
const INDUSTRIES = [
  ...
  'Construction', 'Hospitality', 'Education', 'Logistics', 'Other',
]
...
export function SettingsPage() {
  ...
  async function handleSave() {
    setSaving(true)
    try {
      await api.patch('/employer/me/profile', form)
          ...
          <Section title="Company Information" icon={Building2}>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Founded year">
                <input value={form.founded_year ?? ''} onChange={(e) => set('founded_year', e.target.value)}
                  placeholder="2020" type="number" min="1900" max="2099" className={inputCls} />
              </Field>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Address" className="sm:col-span-2">
                <input value={form.address ?? ''} onChange={(e) => set('address', e.target.value)}
                  placeholder="123 Business Park, Floor 4" className={inputCls} />
              </Field>
              <Field label="City">
