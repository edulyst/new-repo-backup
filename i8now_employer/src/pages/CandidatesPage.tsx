...
export function CandidatesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.get<{ data: Candidate[]; meta: { total: number } }>(`/employer/candidates?${params}`)
  ...
  async function handleShortlist(c: Candidate) {
    try {
      // Create a default shortlist if needed, or add to first existing
      const sRes = await api.get<{ data: { shortlists: { _id: string; name: string }[] } }>('/employer/shortlists')
      let listId: string
      if ((sRes.data?.shortlists ?? []).length === 0) {
        const newList = await api.post<{ data: { shortlist: { _id: string } } }>('/employer/shortlists', { name: 'My Shortlist' })
        listId = newList.data?.shortlist?._id
      } else {
        listId = sRes.data.shortlists[0]._id
      }
      await api.post(`/employer/shortlists/${listId}/candidates/${c.user_id}`, {})
  ...
  async function handleMessage(c: Candidate) {
    try {
      await api.post('/employer/conversations', { worker_id: c.user_id })
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {candidates.map((c) => (
            <div key={c._id} className="flex flex-col rounded-xl border border-zinc-200 bg-white p-5 transition hover:border-zinc-300 hover:shadow-sm">
              ...
              {/* Bio */}
              {c.bio && (
                <p className="mb-3 line-clamp-2 text-xs leading-relaxed text-zinc-400">{c.bio}</p>
              )}
