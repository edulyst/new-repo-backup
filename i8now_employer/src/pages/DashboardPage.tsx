...
import {
  Users, ClipboardList, CalendarCheck, Wallet,
  CalendarDays, Briefcase, ArrowRight, TrendingUp,
  CheckCircle2, Clock, AlertCircle,
} from 'lucide-react'
import { cn } from '@/lib/utils'

interface Wallet { balance: number; total_spent: number }
interface Stats {
  workers: number
  tasks_open: number
...
export function DashboardPage() {
    ...
    async function load() {
      try {
        const [workersRes, tasksRes, interviewsRes, shiftsRes, walletRes] = await Promise.allSettled([
          api.get<{ data: { workers: unknown[] } }>('/employer/workers'),
          api.get<{ data: unknown[]; meta: { total: number } }>('/employer/tasks?status=open&limit=1'),
          api.get<{ data: { interviews: unknown[] } }>('/employer/interviews'),
          api.get<{ data: unknown[]; meta: { total: number } }>('/employer/shifts?status=open&limit=1'),
          api.get<{ data: { wallet: Wallet } }>('/employer/wallet'),
        ])
        ...
        setStats({
          ...
          interviews_upcoming: interviewsRes.status === 'fulfilled'
            ? (interviewsRes.value.data?.interviews?.filter((i: any) => i.status === 'scheduled').length ?? 0) : 0,
  ...
  const kpis = [
    {
      label: 'Active Workers', icon: Briefcase,
      value: stats?.workers ?? 0, format: 'number',
      sub: 'on your team', href: '/workers', trend: 'up',
    },
    {
      label: 'Open Tasks', icon: ClipboardList,
      value: stats?.tasks_open ?? 0, format: 'number',
      sub: 'pending completion', href: '/tasks', trend: 'neutral',
    },
    {
      label: 'Upcoming Interviews', icon: CalendarCheck,
      value: stats?.interviews_upcoming ?? 0, format: 'number',
      sub: 'scheduled', href: '/interviews', trend: 'up',
    },
    {
      label: 'Open Shifts', icon: CalendarDays,
      value: stats?.shifts_open ?? 0, format: 'number',
      sub: 'accepting applications', href: '/shifts', trend: 'neutral',
    },
    {
      label: 'Wallet Balance', icon: Wallet,
      value: stats?.wallet?.balance ?? 0, format: 'money',
      sub: 'available to spend', href: '/wallet', trend: 'up',
    },
    {
      label: 'Total Spent', icon: TrendingUp,
      value: stats?.wallet?.total_spent ?? 0, format: 'money',
      sub: 'payments processed', href: '/wallet', trend: 'neutral',
    },
  ]

  const quickActions = [
    { label: 'Post a new shift',        href: '/shifts',      icon: CalendarDays,  sub: 'Find workers for a role'   },
    { label: 'Browse candidates',        href: '/candidates',  icon: Users,         sub: 'Discover available talent' },
    { label: 'Schedule an interview',    href: '/interviews',  icon: CalendarCheck, sub: 'Arrange a candidate meeting'},
    { label: 'Assign a task',            href: '/tasks',       icon: ClipboardList, sub: 'Delegate work to workers'  },
    { label: 'Process a payment',        href: '/wallet',      icon: Wallet,        sub: 'Pay your workers securely' },
  ]
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-zinc-200 bg-white p-5">
            ...
            <div className="space-y-3">
              <StatusRow icon={CheckCircle2} iconClass="text-emerald-500" label="Workers on duty" value={loading ? '—' : String(stats?.workers ?? 0)} />
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-zinc-900">
                  {loading ? '—' : fmtMoney(stats?.wallet?.balance ?? 0)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-500">Total spent</span>
                <span className="text-sm font-medium text-zinc-700">
                  {loading ? '—' : fmtMoney(stats?.wallet?.total_spent ?? 0)}
                </span>
              </div>
              <div className="mt-3 pt-3 border-t border-zinc-100">
                <Link to="/wallet" className="text-xs font-medium text-zinc-900 hover:underline flex items-center gap-1">
