...
export function WorkersPage() {
  ...
  useEffect(() => {
    api.get<{ data: { workers: Worker[] } }>('/employer/workers')
  ...
  async function handleRate() {
    ...
    setSubmitting(true)
    try {
      await api.post(`/employer/workers/${rateTarget.user_id}/rate`, { rating, comment })
    ...
    <div className="space-y-5 p-6">
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full rounded-lg border border-zinc-200 bg-white py-2.5 pl-9 pr-4 text-sm outline-none transition focus:border-zinc-400"
        />
      </div>
      ...
      {loading ? (
        ...
        <Empty
          ...
          sub="Workers who accept your shifts will appear here."
        />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map((w) => (
