...
import { sendExpoPushMessages } from '../notification/expoPush.send.js'
...
export async function updateEmployerProfile(employerId: string, body: PatchAdminEmployerProfileBody) {
  const patch: {
    ...
    notes?: string | null
    role?: 'owner' | 'admin' | 'recruiter' | 'billing' | 'viewer'
...
export async function patchShiftApplicationStatus(applicationId: string, status: string) {
  ...
  // Real-time push to the worker device when admin changes application status.
  if (prevStatus !== status) {
    const [workerProfile, shift] = await Promise.all([
      WorkerProfileModel.findById(existing.worker_profile_id).lean().exec(),
      ShiftModel.findById(existing.shift_id).lean().exec(),
    ])
    if (workerProfile?.user_id) {
      const workerUser = await UserModel.findById(workerProfile.user_id).lean().exec()
      const expoToken = typeof workerUser?.expo_push_token === 'string' ? workerUser.expo_push_token.trim() : ''
      if (expoToken) {
        const shiftTitle =
          typeof shift?.title === 'string' && shift.title.trim().length > 0 ? shift.title.trim() : 'your shift'
        const title = status === 'confirmed' ? 'Shift confirmed' : 'Shift application updated'
        const body =
          status === 'confirmed'
            ? `Great news! You are confirmed for ${shiftTitle}.`
            : `Your application for ${shiftTitle} is now ${status}.`
        await sendExpoPushMessages([
          {
            ...
            priority: 'high',
            data: {
              eventType: 'shift_application_status_changed',
              status,
      ...
      await NotificationRecord.create({
        ...
        target_user_ids: [String(workerProfile.user_id)],
        recipients_count: 1,
        status: 'delivered',
        sent_at: new Date(),
        created_by: 'system:admin-status',
      })
    }
  }
