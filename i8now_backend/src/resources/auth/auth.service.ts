...
import { sendMail, buildOtpEmailHtml } from '../../utils/mailer.js'
...
export async function requestOtp(body: RequestOtpBody): Promise<RequestOtpResult> {
  ...
  // Send OTP via email when the target is an email address
  if (body.email) {
    const expiresInMinutes = Math.round(OTP_TTL_SEC / 60)
    ...
    sendMail({
      to: body.email,
      subject: 'Your i8now sign-in code',
      html,
      text: `Your i8now sign-in code is: ${otp}\n\nThis code expires in ${expiresInMinutes} minutes.\nNever share this code with anyone.`,
    }).catch((err: unknown) => {
      logger.warn({ err: err instanceof Error ? err.message : String(err) }, '[auth] OTP email failed to send')
    })
  }
