...
export async function applyToShift(userId: string, shiftId: string): Promise<ApplyResult> {
  ...
  // Record + push an immediate "application submitted" update for this worker.
  const shiftTitle = typeof s.title === 'string' && s.title.trim().length > 0 ? s.title.trim() : 'your shift'
  const title = 'Application submitted'
  const body = `Your application for ${shiftTitle} was submitted successfully.`
  ...
  const workerUser = await UserModel.findById(userId).lean().exec()
