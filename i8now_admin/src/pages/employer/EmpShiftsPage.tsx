...
interface Shift {
  _id: string; title: string; location?: string; date?: string
  start_time?: string; end_time?: string; pay_rate?: number
  slots?: number; filled_slots?: number
  status: 'open' | 'filled' | 'in_progress' | 'completed' | 'cancelled'
  description?: string
}
interface Application { _id: string; worker_profile_id: string; status: string; applied_at?: string }
...
function FF({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="space-y-1"><label className="text-xs font-medium text-zinc-700">{label}</label>{children}</div>
}

export function EmpShiftsPage() {
  ...
  function loadShifts() {
    setLoading(true)
    apiGet<{ data: Shift[] }>('/employer/shifts?limit=50')
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    try {
      await apiPost('/employer/shifts', { ...form, pay_rate: Number(form.pay_rate) || 0, slots: Number(form.slots) || 1 })
  ...
  async function loadApplications(shift: Shift) {
    ...
    try {
      const r = await apiGet<{ data: { applications: Application[] } }>(`/employer/shifts/${shift._id}/applications`)
  ...
  async function handleReview(shiftId: string, appId: string, status: string) {
    try {
      await apiPatch(`/employer/shifts/${shiftId}/applications/${appId}`, { status })
      ...
      {showCreate && (
        <Modal title="Post a New Shift" onClose={() => setShowCreate(false)}>
          <div className="space-y-3">
            <FF label="Job title *"><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Warehouse Supervisor" className={inputCls} /></FF>
            <FF label="Location"><input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="e.g. Mumbai, Maharashtra" className={inputCls} /></FF>
            <div className="grid grid-cols-2 gap-3">
              <FF label="Date *"><input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className={inputCls} /></FF>
              <FF label="Slots"><input type="number" min="1" value={form.slots} onChange={(e) => setForm({ ...form, slots: e.target.value })} className={inputCls} /></FF>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <FF label="Start time"><input type="time" value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })} className={inputCls} /></FF>
              <FF label="Pay rate"><input type="number" value={form.pay_rate} onChange={(e) => setForm({ ...form, pay_rate: e.target.value })} placeholder="0" className={inputCls} /></FF>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button onClick={handleCreate} disabled={saving} className={primaryBtn}>{saving ? 'Creating…' : 'Post Shift'}</button>
            </div>
          </div>
        </Modal>
      )}
                    ...
                    {app.status === 'applied' && (
                      <>
                        <button onClick={() => handleReview(selected._id, app._id, 'confirmed')}
                          className="rounded-md bg-emerald-600 px-2.5 py-1 text-[10px] font-medium text-white hover:bg-emerald-700">Confirm</button>
                        <button onClick={() => handleReview(selected._id, app._id, 'rejected')}
                          className="rounded-md border border-zinc-200 px-2.5 py-1 text-[10px] font-medium text-zinc-600 hover:bg-zinc-100">Reject</button>
                      </>
                    )}
                  </div>
                </div>
