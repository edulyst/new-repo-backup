...
interface Task {
  ...
  priority?: 'low' | 'medium' | 'high'; worker_id?: string
...
export function EmpTasksPage() {
  ...
  function loadTasks() {
    setLoading(true)
    apiGet<{ data: Task[] }>('/employer/tasks?limit=50')
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    try {
      await apiPost('/employer/tasks', { ...form, due_date: form.due_date || undefined, worker_id: form.worker_id || undefined })
  ...
  async function handleStatusChange(id: string, status: string) {
    try {
      await apiPatch(`/employer/tasks/${id}`, { status })
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await apiDelete(`/employer/tasks/${id}`)
          ...
          <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Prepare weekly report" className={inputCls} />
              </div>
              <div className="space-y-1">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={handleCreate} disabled={saving} className={primaryBtn}>{saving ? 'Creating…' : 'Create Task'}</button>
              </div>
            </div>
          </div>
        </div>
